...
def load_an_image(path):
    img_LR = read_img(None, path)
    H, W, C = img_LR.shape

    # BGR to RGB, HWC to CHW, numpy to tensor
    if img_LR.shape[2] == 3:
        img_LR = img_LR[:, :, [2, 1, 0]]
    img_LR = torch.from_numpy(np.ascontiguousarray( np.expand_dims( np.transpose(img_LR, (2, 0, 1)) , axis=0) )).float()

    return {'LQ': img_LR, 'LQ_path': path}
